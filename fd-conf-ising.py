...
def par_relax(initial_par, final_par, t, t0, tau):
    ...
    if final_par > 0:
        ...
        if (initial_par-final_par) < 0:
            ...
            return np.heaviside(t0-t, t) * initial_par + np.heaviside(t-t0, 0) * ((final_par-initial_par) * np.exp(-((t-t0)**2)/(tau*20)) +initial_par)
...
def fd_evolve(N, time_pars, h_pars, c1, probe_pars, site):
    ...
    for j in range(len(js)):
        ...
        for i in range(time_pars['nt']-1):
            ...
            for n in range(N):
                ...
                response_array[n, j, i+1] = np.conj(np.transpose(PauliSparse[n] @psi[i,:])) @  psi[i,:]     
...
probe_pars = {'j_points': nj,'sigma': sigma, 'amp': amp, 'w': w, 'mapping': 'GAUSSIAN', 'opp': 'Z', 'site': 0}
h_pars = {'model':'LF', 'h_l0': h_l0}
c1 = {'J':1, 'h_t':0.25, 'h_l':h_l}

A, js, hit_arr,ct_l = fd_evolve(N, time_pars, h_pars, c1, probe_pars, site)
np.savez('response_FD_NEW.npz', arr1 = A, arr2 = js, arr3 = hit_arr, arr4 = ct_l)
...
def f_derivatives(response, j_points, N, tax, hit_arr, damping_power):
    ...
    for j in range(len(j_points)):
        ...
        for n in range(N):
            ...
            A_ft[n,j,:] = np.fft.fftshift(np.fft.fft(A_damped[n,j,:]))


    
    for j in range(1,len(j_points)):
        
        hofw[j, :] = np.fft.fftshift(np.fft.fft(hit_arr[j, :]))
...
wax, a_FT, A_damped, corr_FD, hofw = f_derivatives(A, js, N, tax, hit_arr, 10)
